'''
Project 4: Sudoku Master Part 1
...
class Board(object):
    ...
    def is_consistent(self) -> bool:
        '''detects duplicate values, but not which values were duplicates'''
        for group in self.groups:
            used_nums = set()
            for tile in group:
                if tile.value in CHOICES:
                    if tile.value in used_nums:
                        return False
                    else:
                        used_nums.add(tile.value)
        return True
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            ...
            for tile in group:
                if tile.value in CHOICES:
                    values_used.add(tile.value)
            for tile in group:
                if tile.value in UNKNOWN:
                    if tile.remove_candidates(values_used) is True:
                        changed = True
        return changed

    def hidden_single(self):
        '''Takes the only remaining possibility for a tile and places it in that tile.'''
        progress = False
        for group in self.groups:
            ...
            for num in leftovers:
                ...
                if counter == 1:
                    for tile in group:
                        if (tile.value not in CHOICES) and (num in tile.candidates):
                            tile.set_value(num)
                            progress = True
    ...
    def solve(self) -> bool:
        ...
        if self.is_complete():
            ...
            for tile in chosen.candidates:
                chosen.set_value(tile)
                if self.solve():
                    return True
                else:
                    self.set_tiles(save)
    ...
    def min_choice_tile(self) -> Tile:
        ...
        with value UNKNOWN.
        """
        smallest = set(CHOICES)
        min_tile = None
        for group in self.tiles:
            for tile in group:
                if tile.value == UNKNOWN:
                    if len(tile.candidates) < len(smallest):
                        smallest = tile.candidates
                        min_tile = tile
        return min_tile
